-- Version update:
-- 1) Added new class: Wall to take care  the calculation part
-- 2) Improved performance
-- 3) Computer is now able to count up to two rebounds and moves to the middle after their turn
...
function serveCalculation(sy, vx, vy)
    ...
    if vy > 0 then
        targetY = Virtual_Height / 2 + distance * m
    elseif vy < 0 then
        targetY = Virtual_Height / 2 - distance * m
    else
        targetY = sy
...
function love.update(dt)
    if gameState == 'serve' then
        ...
        -- to toggle calculation and movement
        served = true
        player1Turn = false
        player2Turn = false
    ...
    elseif gameState == 'play' then
        ...
        if ball:collide(player1) then
            ...
            if playerState ~= 'manual' then
                sy, vx, vy = ball:getParameters()
                targetY = wall:calculation(sy, vx, vy)
                player1Turn = false
                player2Turn = true
            end
        end
        ...
        if playerState ~= '1' then
            if player1Turn then
                player1:move(targetY, dt)
                if playerState ~= '2' then
                    player2:move(Virtual_Height/2, dt)
                end
                player2Turn = false
            elseif player2Turn then
                player1:move(Virtual_Height/2, dt)
                if playerState ~= '2'then
                    player2:move(targetY, dt)
                end
                player1Turn = false
...
function love.draw()
    ...
    if gameState == 'pick' then
        ...
        love.graphics.printf('Press M to play with yourself or others', 0, 40, Virtual_Width, 'center')
        displaySide()
    end
