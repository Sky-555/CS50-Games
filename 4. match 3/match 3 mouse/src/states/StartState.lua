...
function StartState:update(dt)
    ...
    if not self.pauseInput then
        ...
        if love.mouse.wasPressed(1) then
            ...
            if x > 230 and x < VIRTUAL_WIDTH - 230 then
                if y > VIRTUAL_HEIGHT / 2 + 20 and y < VIRTUAL_HEIGHT / 2 + 35 then
                    self.currentMenuItem = 1
                    Timer.tween(1, {[self] = {transitionAlpha = 1}}):finish(function()
                        gStateMachine:change('begin-game', {level = 1}) end)
            ...
            if x > 210 and x < VIRTUAL_WIDTH - 210 then
                if y > VIRTUAL_HEIGHT / 2 + 45 and y < VIRTUAL_HEIGHT / 2 + 60 then
                    love.event.quit()
                    self.pauseInput = true
                end
            end
