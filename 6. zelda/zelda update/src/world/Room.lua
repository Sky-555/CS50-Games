...
function Room:generateEntities()
    ...
    for k, entity in pairs(self.entities) do
        entity.stateMachine = StateMachine {
            ['walk'] = function() return EntityWalkState(entity, self.dungeon) end,
            ['idle'] = function() return EntityIdleState(entity, self.dungeon) end
        }
...
function Room:generateObjects()
    ...
    for i = 1, math.random(2, 5) do
        ...
        pot.state = 'pot' .. tostring(math.random(4))

        pot.onCollide = function(player)
            if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
                if not player.occupied then
