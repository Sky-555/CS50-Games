...
function Entity:collides(target)
    return not (self.x + self.width < target.x + 1 or self.x + 1 > target.x + target.width or
                self.y + self.height < target.y + 1 or self.y + 1 > target.y + target.height)
end
...
function Entity:dropHeart(object)
    if math.random(5) == 1 then
        ...
        -- 5050 chance to spawn half heart, and flip it if that's the case
        ...
        if not heart.spawned then
            table.insert(object, heart)
        end
    end
end
